# some functions taken from http://www.documentcloud.org/javascripts/lib/inflector.js
window.Inflector =
  # Delegate to the ECMA5 String.prototype.trim function, if available.
  trim: (s) ->
    if s.trim then s.trim() else s.replace(/^\s+|\s+$/g, '')
  ...
  sluggify: (s) ->
    @trim(@removeAccents(s).replace(/['"]+/g, '').replace(/\W+/g, ' ')).toLowerCase().replace(/\s+/g, '-')
